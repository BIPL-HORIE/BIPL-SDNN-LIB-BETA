...
@brief 独自企画パラメータのロードと出力に関するクラス，実装部
@author Kazumasa HORIE
@date 2015/12/1~
@version 0.8
*/
#include "BIPL_LIB_PARAMETER_BASE.h"
...
/*!
@brief 独自企画パラメータのPIMPL
@author Kazumasa HORIE
@date 2015/12/1~
@version β1.0
*/
class bipl::parameters::PARAMETERS::PARAMETERS_ROOT
{
private:
...
public:
	...
	@param[in] parameter_file パラメータが書かれたファイル
	...
	*/
	...
	void LoadFile(stream_type &stream)
	{
		...
		while (getline(stream, line_buffer))
		{
			...
			{
				bipl::lexial::DeleteCharactor(line_buffer, "<>{");//スペース削除
	...
	*/
	...
	/*! @brief パラメータを読む
	@note 見つからなかった（かつdefaultが無い）場合は，””（空）で返す．
	@note 本当はdefault_parameterをPARAMETERS内で処理したかったのだが，LoadFileする関係上ここにおくほかなかった．
	@param[out] out_parameter_content 読み込んだパラメータの内容
	@param[in] parameter_name 読みたいパラメータの名前
	@param[in] default_parameter デフォルト設定，パラメータが存在しなかった場合これが返りさらに登録を行う．
	@return なし．エラー発生時はthrow．
	*/
...
template void bipl::parameters::PARAMETERS::PARAMETERS_ROOT::LoadFile(std::ifstream &stream);
template void bipl::parameters::PARAMETERS::PARAMETERS_ROOT::LoadFile(std::stringstream &stream);

//PARAMETERS
bipl::parameters::PARAMETERS::PARAMETERS() :pimpl_(new PARAMETERS_ROOT){}
bipl::parameters::PARAMETERS::~PARAMETERS()
{
}

void bipl::parameters::PARAMETERS::LoadFile(const std::string parameter_file_name)
{
	try {
...
void bipl::parameters::PARAMETERS::LoadFile(std::stringstream &parameter_stream)
{
	try {
...
void bipl::parameters::PARAMETERS::OverwriteParameter(const std::string parameter_name, const std::string &parameter_content)
{
	try {
...
template<>
void bipl::parameters::PARAMETERS::OverwriteParameter(const std::string parameter_name, const std::vector<std::string> parameter_content)
{
	try {
...
void bipl::parameters::PARAMETERS::OverwriteParameter(const std::string parameter_name, const std::vector<content_type> parameter_content)
{
	try {
		if (parameter_content.size() == 0)
...
template void bipl::parameters::PARAMETERS::OverwriteParameter(const std::string parameter_name, const std::vector<int> parameter_content);
template void bipl::parameters::PARAMETERS::OverwriteParameter(const std::string parameter_name, const std::vector<double> parameter_content);
template void bipl::parameters::PARAMETERS::OverwriteParameter(const std::string parameter_name, const std::vector<unsigned int> parameter_content);

template<>
void bipl::parameters::PARAMETERS::ReadParameter(std::string &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement)
{
...
template<typename content_type>
void bipl::parameters::PARAMETERS::ReadParameter(content_type &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement)
{
	std::string content_buffer;
	...
	if (content_buffer == "")
		out_parameter_content = 0;
	else if (lexial::CountCharactor(content_buffer, "+-*/^().")) //演算子持ち + 小数
	{
...
//テンプレート関数の実体化
template void bipl::parameters::PARAMETERS::ReadParameter(int &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement);
template void bipl::parameters::PARAMETERS::ReadParameter(double &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement);
template void bipl::parameters::PARAMETERS::ReadParameter(unsigned int &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement);



template<>
void bipl::parameters::PARAMETERS::ReadParameter(std::vector<std::string> &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement)
{
	try
	{
...
template<typename content_type>
void bipl::parameters::PARAMETERS::ReadParameter(std::vector<content_type> &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement)
{
	try
	{
		...
		for (auto content_buffer_iterator = content_buffer.begin(); content_buffer_iterator != content_buffer.end(); ++content_buffer_iterator)
		{
			...
			if (lexial::CountCharactor(*content_buffer_iterator, "+*-/^().[]")) //演算子持ち+小数
			{
...
template void bipl::parameters::PARAMETERS::ReadParameter(std::vector<int> &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement);
template void bipl::parameters::PARAMETERS::ReadParameter(std::vector<double> &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement);
template void bipl::parameters::PARAMETERS::ReadParameter(std::vector<unsigned int> &out_parameter_content, const std::string parameter_name, const std::string default_parameter_content, judgement::JUDGEMENT *judgement);

void bipl::parameters::PARAMETERS::ListParameter(std::string &out_parameter_list)const
{
	try {
